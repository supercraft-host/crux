...
async function main() {
  const crux = CruxClient.forPlayer(url, projectId, envId, apiKey);

  const auth = await crux.loginAnonymous();
  console.log(`logged in as ${auth.player_id}`);

  const score = Math.floor(Math.random() * 10_000);
  await crux.submitScore(leaderboard, auth.player_id, score);
  console.log(`submitted ${score}`);

  const top = await crux.getTop(leaderboard, 10);
  console.log(`top ${top.length}:`);
  top.forEach(e => console.log(`  #${e.rank.toString().padStart(2)} ${e.player_id} → ${e.score}`));

  const me = await crux.getPlayerStanding(leaderboard, auth.player_id);
  console.log(`my standing: ${me?.rank ?? "unranked"}`);

  const neighbours = await crux.getAroundPlayer(leaderboard, auth.player_id, 2);
