...
GSB has three authentication modes. Every SDK in this repo speaks all
...
See also: [Guest login and account upgrade — gsb.supercraft.host](https://gsb.supercraft.host/blog/guest-login-and-account-upgrade).
...
See [Roblox HttpService → external backend on gsb.supercraft.host](https://gsb.supercraft.host/blog/roblox-httpservice-external-backend)
...
```gdscript
GSB.init_player(base_url, project_id, env_id, api_key)
GSB.init_server(base_url, project_id, env_id, server_token)
```
...
```lua
local gsb = GSB.init(projectId, serverToken, envId)
```

## Rotating keys

If a key leaks, rotate it immediately from the GSB dashboard
([gsb.supercraft.host](https://gsb.supercraft.host/) → your project →
...
SDKs verify only that the token *parses* — they trust GSB to have
...
`https://gsb.supercraft.host/.well-known/jwks.json`.
