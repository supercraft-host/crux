...
## Verify a player JWT from a trusted dedicated server. The Crux backend must
## expose this runtime endpoint; the server token remains the authenticating
## credential and the player JWT is supplied only as request data.
func verify_player_token(player_jwt: String) -> Dictionary:
	if _server_token.is_empty():
		return {"error": "server_token_required"}
	return await _request(
		"POST",
		"/v1/projects/%s/auth/verify" % _project_id,
		{"token": player_jwt, "environment_id": _env_id},
		_server_auth(),
	)
...
func submit_score(leaderboard_id: String, pid: String, score: float, metadata: Dictionary = {}) -> Dictionary:
	return await _request("POST", _env("/leaderboards/%s/scores" % leaderboard_id),
...
## balance_adjustments: [{currency_key, amount}]
## inventory_adjustments: [{item_key, quantity, metadata}]
...
func heartbeat(server_id: String) -> Dictionary:
	return await _request("POST", _env("/servers/heartbeat"), {"server_id": server_id}, _server_auth())


## Deregister on clean shutdown.
func deregister_server(server_id: String) -> Dictionary:
	return await _request("POST", _env("/servers/deregister"), {"server_id": server_id}, _server_auth())
...
func _send(method: String, path: String, body: Dictionary, auth_header: String) -> Variant:
	...
	for attempt in range(MAX_RETRIES + 1):
		...
		if err != OK:
			http.queue_free()
			return {"error": "transport_error", "transport_code": err, "path": path}

		var response = await http.request_completed
		http.queue_free()
		var response_code: int = response[1]
		var body_raw: PackedByteArray = response[3]
		var body_str := body_raw.get_string_from_utf8()

		if response_code in [429, 503] and attempt < MAX_RETRIES:
			await get_tree().create_timer(backoff).timeout
			backoff *= 2.0
			continue

		if response_code >= 400:
			var parsed_error := _parse_json(body_str)
			if parsed_error.is_empty():
				parsed_error = {"message": body_str}
			parsed_error["error"] = str(parsed_error.get("code", "http_error"))
			parsed_error["status"] = response_code
			parsed_error["path"] = path
			return parsed_error
	...
	return {"error": "max_retries_exceeded", "path": path}
