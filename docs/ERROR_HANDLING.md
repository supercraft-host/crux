...
Every Crux SDK exposes failures in the language-native way (exceptions
...
| `500 Internal Server Error` | Crux is unhappy. | Surface the error; retry only if idempotent. |
| `503 Service Unavailable` | Transient - Crux is degraded. | SDKs auto-retry up to 3× with exponential backoff. |
...
pass it, Crux compares the version to the one currently stored and
...
If you don't pass `version`, Crux performs a last-write-wins update.
